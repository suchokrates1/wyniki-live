...
(function initDarkMode() {
  ...
  if (toggle) {
    toggle.addEventListener('click', () => {
      ...
      if (icon) {
        icon.textContent = newTheme === 'dark' ? 'ÔśÇ´ŞĆ' : '­čîÖ';
      }
      
      // Toast notification
      if (typeof window.showToast === 'function') {
        window.showToast('info', 'Tryb zmieniony', `W┼é─ůczono tryb ${newTheme === 'dark' ? 'ciemny' : 'jasny'}`);
      }
    });
    ...
    if (icon) {
      icon.textContent = currentTheme === 'dark' ? 'ÔśÇ´ŞĆ' : '­čîÖ';
    }
  }
})();
...
(function () {
  ...
  function updateUnoStatusSummary() {
        ...
        if (peak.next_reset) {
          try {
            ...
            if (diffMs > 0) {
              ...
              if (minutes > 0) {
                hourlyText += ` ÔÇó reset za ${minutes}min`;
              } else {
                hourlyText += ` ÔÇó reset za ${seconds}s`;
              }
            }
          } catch (e) {
    ...
    if (unoAutoDisabled) {
      if (unoAutoDisabledReason) {
        unoAutoDisabled.textContent = `Automatycznie wy┼é─ůczono: ${unoAutoDisabledReason}`;
        unoAutoDisabled.hidden = false;
      } else if (!unoRequestsEnabled) {
        unoAutoDisabled.textContent = 'Zapytania wy┼é─ůczone r─Öcznie.';
        unoAutoDisabled.hidden = false;
      } else {
  ...
  function applyPluginToggle(enabled) {
    ...
    if (pluginToggleStatus) {
      pluginToggleStatus.textContent = value
        ? 'Wtyczka jest u┼╝ywana i jej komunikaty s─ů akceptowane.'
        : 'Komunikaty od wtyczki s─ů ignorowane.';
    }
  }
  ...
  function applyUnoRateLimit(info) {
    ...
    if (!info || typeof info !== 'object') {
      ...
      if (unoRateLimitHeader) {
        unoRateLimitHeader.textContent = 'ÔÇö';
        unoRateLimitHeader.hidden = true;
      }
      if (unoRateLimitUpdated) {
        unoRateLimitUpdated.textContent = 'ÔÇö';
      }
      if (unoRateLimitReset) {
        unoRateLimitReset.textContent = 'ÔÇö';
      }
      if (unoRateLimitMeta) {
    ...
    if (unoRateLimitUpdated) {
      unoRateLimitUpdated.textContent = rateLimit.updated || 'ÔÇö';
    }
    if (unoRateLimitReset) {
      let resetLabel = 'ÔÇö';
  ...
  async function handleUnoActivityReset(event) {
    ...
    try {
      ...
      applyUnoSystemData(data);
      setFeedback('Przywr├│cono normalny tryb zapyta┼ä UNO.', 'success');
    } catch (error) {
      setFeedback(error.message, 'error');
    }
  }
  ...
  async function handleUnoPollerFormSubmit(event) {
    ...
    if (!Number.isFinite(limitValue) || limitValue < 0 || !Number.isInteger(limitValue)) {
      setFeedback('Limit na godzin─Ö musi by─ç liczb─ů ca┼ékowit─ů Ôëą 0.', 'error');
      return;
    }
    ...
    if (!Number.isFinite(thresholdValue) || thresholdValue < 0 || thresholdValue > 100) {
      setFeedback('Pr├│g spowolnienia musi by─ç liczb─ů z zakresu 0-100.', 'error');
      return;
    }
    ...
    if (!Number.isFinite(factorValue) || factorValue < 1 || !Number.isInteger(factorValue)) {
      setFeedback('Wsp├│┼éczynnik spowolnienia musi by─ç liczb─ů ca┼ékowit─ů Ôëą 1.', 'error');
      return;
    }
    const sleepValue = unoSlowdownSleepInput ? unoSlowdownSleepInput.valueAsNumber : NaN;
    if (!Number.isFinite(sleepValue) || sleepValue < 0) {
      setFeedback('Dodatkowa pauza musi by─ç liczb─ů nieujemn─ů.', 'error');
      return;
    }
    ...
    try {
      ...
      applyUnoSystemData(data);
      setFeedback('Konfiguracja limit├│w UNO zapisana.', 'success');
    } catch (error) {
      setFeedback(error.message, 'error');
    }
  }
      ...
      if (field.name === 'flag_url') {
        ...
        input.addEventListener('input', () => {
          ...
          if (input.value.trim()) {
            ...
            const flagCellToUpdate = row.querySelector('.flag-cell');
            if (flagCellToUpdate) {
              flagCellToUpdate.innerHTML = '<span style="color: var(--muted); font-size: 11px;">ÔÇö</span>';
            }
          }
        });
      }
  ...
  async function resetCourtState(kortId) {
    ...
    if (typeof window.showConfirmDialog === 'function') {
      confirmed = await window.showConfirmDialog(
        'Zresetowa─ç kort?',
        `Czy na pewno wyzerowa─ç stan kortu ${kortId}? Ta akcja jest nieodwracalna.`,
        { confirmText: 'Resetuj', cancelText: 'Anuluj', danger: true }
      );
    } else {
      confirmed = window.confirm(`Czy na pewno wyzerowa─ç stan kortu ${kortId}?`);
    }
  ...
  async function deleteEntry(row) {
    ...
    if (typeof window.showConfirmDialog === 'function') {
      confirmed = await window.showConfirmDialog(
        'Usun─ů─ç rekord?',
        'Czy na pewno usun─ů─ç ten rekord z historii? Ta akcja jest nieodwracalna.',
        { confirmText: 'Usu┼ä', cancelText: 'Anuluj', danger: true }
      );
    } else {
      confirmed = window.confirm('Czy na pewno usun─ů─ç ten rekord?');
    }
    ...
    card.innerHTML = `
      ...
      ${isEmpty ? `
        <div class="court-card__empty-state">
          <div class="court-card__empty-icon">­čÄż</div>
        ...
        <div class="court-card__match">
          ...
          ${court.match_time?.seconds ? `
            <div class="court-card__time">
              ÔĆ▒´ŞĆ ${formatMatchTime(court.match_time.seconds)}
            </div>
          ` : ''}
        </div>
      `}
    ...
    dashboardRefreshBtn.addEventListener('click', async () => {
      try {
        ...
        if (Array.isArray(courts)) {
          renderDashboard(courts);
          if (typeof window.showToast === 'function') {
            window.showToast('success', 'Od┼Ťwie┼╝ono', 'Dashboard zaktualizowany');
          }
        }
      } catch (error) {
        setFeedback(error.message, 'error');
  ...
  function showKeyboardHelp() {
    const helpText = `
      <strong>Skr├│ty klawiszowe:</strong><br><br>
      <code>Ctrl+1/2/3/4</code> Ôćĺ Otw├│rz kort w nowej karcie<br>
      <code>Ctrl+D</code> Ôćĺ Przejd┼║ do dashboardu<br>
      <code>?</code> Ôćĺ Poka┼╝ t─Ö pomoc<br>
      <code>Esc</code> Ôćĺ Zamknij moda┼éy
    `;
    
    if (typeof window.showToast === 'function') {
      window.showToast('info', 'Skr├│ty klawiszowe', helpText.replace(/<br>/g, '\n').replace(/<[^>]*>/g, ''), 8000);
    } else {
  ...
  if (!localStorage.getItem('shortcuts-help-shown')) {
    setTimeout(() => {
      if (typeof window.showToast === 'function') {
        window.showToast('info', 'Wskaz├│wka', 'Naci┼Ťnij ? aby zobaczy─ç skr├│ty klawiszowe', 6000);
      }
  ...
  function filterPlayers() {
    ...
    if (playersSearchInput) {
      ...
      if (searchTerm || groupFilter) {
        playersSearchInput.placeholder = `­čöŹ Pokazano ${visibleCount}/${total} graczy`;
      } else {
        playersSearchInput.placeholder = '­čöŹ Szukaj gracza (nazwa, lista, grupa)...';
      }
    }
  }
  ...
  function sortPlayers(column) {
    ...
    if (typeof window.showToast === 'function') {
      window.showToast('info', 'Sortowanie', `Posortowano po: ${column} (${sortState.direction === 'asc' ? 'ÔćĹ' : 'Ôćô'})`, 2000);
    }
  }
