...
(function () {
  ...
  function createCourtRow(court) {
    ...
    pauseButton.className = 'pause-polling';
    const isPaused = court.polling_paused === true;
    pauseButton.textContent = isPaused ? 'Wznów polling' : 'Wstrzymaj polling';
    pauseButton.dataset.paused = String(isPaused);
    pauseButton.style.marginRight = '4px';
    if (isPaused) {
    ...
    resetButton.style.marginRight = '4px';
  ...
  async function toggleCourtPolling(kortId, buttonElement) {
    if (!kortId) {
      setFeedback('Nie udało się odczytać identyfikatora kortu.', 'error');
      return;
    }
    const isPaused = buttonElement.dataset.paused === 'true';
    const newState = !isPaused;
    const action = newState ? 'wstrzymać' : 'wznowić';
    
    if (!window.confirm(`Czy na pewno ${action} polling dla kortu ${kortId}?`)) {
      return;
    }
    
    try {
      const data = await requestJson(`/api/admin/court/${encodeURIComponent(kortId)}/polling`, {
        method: 'PUT',
        body: JSON.stringify({ paused: newState })
      });
      
      buttonElement.dataset.paused = String(data.paused);
      buttonElement.textContent = data.paused ? 'Wznów polling' : 'Wstrzymaj polling';
      buttonElement.style.backgroundColor = data.paused ? '#28a745' : '#ffc107';
      
      const statusText = data.paused ? 'wstrzymany' : 'wznowiony';
      setFeedback(`Polling dla kortu ${kortId} został ${statusText}.`, 'success');
    } catch (error) {
      setFeedback(error.message, 'error');
    }
  }
  ...
  function handleCourtsTableClick(event) {
    ...
    if (target.classList.contains('pause-polling')) {
      toggleCourtPolling(row.dataset.kortId || '', target);
    } else if (target.classList.contains('reset-court')) {
